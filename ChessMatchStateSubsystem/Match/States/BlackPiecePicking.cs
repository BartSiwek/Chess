...
{
    internal class BlackPiecePicking : IMatchState
    {
        #region Storage

        private static BlackPiecePicking s_instance;
        ...
        private BlackPiecePicking(ChessMatchState owner)
        {
            m_owner = owner;
        }

        public static BlackPiecePicking GetMatchState(ChessMatchState owner)
        {
            if (s_instance == null)
            {
                s_instance = new BlackPiecePicking(owner);
            }
            else
            {
        ...
        public IMatchState Update(GameTime gameTime)
        {
            ...
            if (m_owner.InputManager.IsLeftMouseButtonDown())
            {
                ...
                if (m_owner.GraphicsManager.IsMouseIn(mousePosition))
                {
                    ...
                    if (m_owner.Match.TrySelect(ChessPlayer.Black, pickedTile))
                    {
                        m_owner.AudioManager.PlaySound(SoundKey.PieceUp);
                        return BlackPieceMoving.GetMatchState(m_owner);                        
                    }
                }
            }
